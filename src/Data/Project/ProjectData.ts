...
const PROJECT_DATA = [
    ...
    result: [
      {
        ...
        content:
          "✅ Next의 Hybrid Rendering 방식을 통해 SSR이 어떤 방식으로 이뤄지고 React의 CSR 방식과는 극명하게 다르다는 걸 알게 된 프로젝트입니다. 초기 로딩 속도가 SSR에 상대적으로 느리고 SEO에 취약한 CSR 과는 달리 서버에서 미리 JavaScript코드를 제외하여 만들어진 페이지를 클라이언트로 보내주기 때문에 SEO에 이점이 있고 Hybrid Rendering 방식을 통해서 초기 페이지를 불러올 때는 SSR 방식을 이용하고 이후 페이지 전환과 컴포넌트 변경 시에는 CSR 방식을 통하여 렌더링을 할 수 있다는 점은 신선한 충격으로 다가왔습니다.",
      },
      {
        title:
          "실시간 데이터 통신을 위한 방법: 웹 소켓을 이용한 실시간 채팅 및, 게임 구현",
        content:
          "✅ 소켓 통신과 REST API를 통한 데이터 통신에는 어떤 차이점이 있는지를 확연하게 알게 되었습니다. REST API를 이용한 통신은 단방향성 통신이라 실시간 채팅을 구현할 때 주기적으로 데이터 요청을 보내주어야 하지만 소켓 통신은 양방향 통신으로 연결된 상태를 유지하며 실시간성에서 뛰어난 모습을 보이므로 채팅이나 유저 간의 게임 같은 실시간성이 중요한 기능에 적합하다는 걸 몸소 느꼈습니다.",
      },
      {
        title:
          "서로 다른 개발환경: Docker를 이용한 개발환경 통합 및 효율성 증대",
        content:
          '✅ "Docker"를 이용하여 개발 환경을 통합하였습니다. 팀 프로젝트에서 각 팀원끼리의 개발 환경은 다르므로 Docker를 통해서 BackEnd, FrontEnd 테스트 환경을 통일하였습니다. 그 결과로 작성한 코드를 테스트할 때 시간이 단축되고 개발의 효율성이 증대하였습니다.',
      },
      {
        title:
    ...
    result: [
      {
        ...
        content:
          "✅ 이전 프로젝트에서는 반응형을 적용하지 않은 디자인이었지만 새로운 버전에서는 웹, 앱을 둘 다 대응할 수 있도록 전체 디자인을 변경하였고 FrontEnd의 스택이었던 Next를 React로 바꿔서 프로젝트를 진행하였습니다. 기존 Next에서는 CSR, SSR, SSG와 같은 다양한 종류의 렌더링 방식을 제공하지만 프로젝트의 기능들과 활용성을 생각해 보면 정보를 노출하여서 SEO에 최적화가 필요한 것도 아닌 CSR만 이용해도 문제없겠다고 판단하여 React로 리팩토링을 진행하였습니다.",
      },
      {
        title:
          '메세지 데이터 저장을 통한 사용자 경험 향상: "Firebase"를 이용한 메세지 데이터 저장',
        content:
          '✅ 기존 "Tscenping"에서는 채팅 메시지 데이터를 저장하지 않고 전역변수로 관리하여 새로고침, 채팅방 나가기, 로그아웃을 하면 메세지가 사라진다는 단점이 있었습니다. 메시지 데이터가 휘발성인 채팅 기능은 완벽한 기능이 아니라고 판단하여 이를 보완하기 위해 Firebase에 연결하여 모든 메시지를 저장하고 유저가 로그아웃을 하거나 강제로 브라우저를 종료하거나 새로고침을 해도 휘발성인 메시지가 아닌 Firebase에서 메시지 데이터를 불러와서 사용자의 경험을 향상시켰습니다.',
      },
      {
        title:
          "데이터 캐싱으로 효율적인 데이터 호출: ReactQuery를 활용한 채팅방 리스트 및 랭킹 리스트 관리",
        content:
          "✅ React Query의 서버상태관리 및 캐싱 기능을 이용하여 API호출 상태를 관리하고 데이터 캐싱을 통해 서버 호출을 최소화 시켰습니다.기존 Tscenping프로젝트보다 상태관리코드를 400줄에서 300줄로 간소화 시키고 데이터 캐싱으로 서버의 부담을 줄였습니다.",
      },
      {
        title:
  ...
  },
  {
    ...
    result: [
      ...
      },
      {
        ...
        content:
          "✅ 웹 환경에만 대응하는게 아닌 앱 환경에도 대응하기 위해 PWA를 적용시켜 다양한 플랫폼에도 접근하기 편하고 문제없이 동작하는 환경을 구축하였습니다.",
      },
      {
        title:
          "API 성능 향상을 위한 최적화 전략 : 서버 설정을 통한 사용자 UX개선",
        content:
          "✅ API호출 응답시간을 획기적으로 줄였습니다. 기존 600msAPI응답 시간과 120ms걸리는 호출시간을 서버 최적화를 통해 6배 빠른 100ms, 20ms정도로 단축하여 사용자 경험을 향상 시켰습니다.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    result: [
      ...
      {
        title:
          "로그인 인증: JWT, Secure Cookie, Local Storage을 이용한 유저 로그인 관리",
        content:
      ...
      {
        title:
          "사용자 중심의 프로젝트: 불편함을 해소하는 경험으로부터 얻은 교훈",
        content:
